...
// If we have a list = seg + {value}, and:
// - We pick all length(list) - 1 values, we get seg;
// - We drop all length(list) - 1 values, we get {value};
// - We get the element at position length(list) - 1, we get value.
...
// Same lemma as "append_take_drop" except "n" maybe be equal to "length(xs)"
...
// Move the first element of the "to reverse" (right) list to the "reversed" list (left)
lemma void move_node_right_left<t>(t oldHeadVal, list<t> left, list<t> right, t tailVal)
  requires length(right) > 0;
  ensures append(cons(oldHeadVal, reverse(left)), append(right, cons(tailVal, nil))) ==
          append(cons(oldHeadVal, reverse(cons(head(right), left))), append(tail(right), cons(tailVal, nil)));
{
  append_assoc(cons(oldHeadVal, reverse(left)), right, cons(tailVal, nil));
  append_assoc(cons(oldHeadVal, reverse(left)), cons(head(right), nil), append(tail(right), cons(tailVal, nil)));
  switch (right) {
    case nil: assert false;
    case cons(x, xs0): assert right == cons(head(right), tail(right));
  }
  assert right == append(cons(head(right), nil), tail(right));
}
@*/
